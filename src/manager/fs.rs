...
pub struct FsManager {
    ...
    /// - cgroupsv1: <subsystem> -> <path>
    /// - cgroupsv2: "" -> <path>
    paths: HashMap<String, String>,
    /// Cgroup mountpoints read from `/proc/self/mountinfo`.
    mounts: HashMap<String, String>,
    /// Base path of the cgroup filesystem, the complete path would be:
    /// - cgroupsv1: "/sys/fs/cgroup/<subsystem>/<base>"
    /// - cgroupsv2: "/sys/fs/cgroup/<base>"
    base: String,
    /// Cgroups through cgroupfs.
    pub(crate) cgroup: Cgroup,
}

impl FsManager {
    /// Create an instance of FsManager, and create the cgroups at the
    /// specified base path.
    pub fn new(base: &str) -> Result<Self> {
        let paths = parse_cgroup_subsystems()?;
        let mounts = parse_cgroup_mountinfo(&paths)?;
        let cgroup = Cgroup::new(hierarchies::auto(), base).map_err(Error::Cgroupfs)?;
    ...
    /// Create an instance of FsManager, but don't create any cgroups.
...
impl Manager for FsManager {
    fn apply(&self, tgid: CgroupPid) -> Result<()> {
        self.cgroup.add_task_by_tgid(tgid).map_err(Error::Cgroupfs)
    }
    ...
    fn destroy(&mut self) -> Result<()> {
        self.cgroup.delete().map_err(Error::Cgroupfs)
    }
...
mod tests {
    ...
    const TEST_BASE: &str = "cgroupsrs/fs";
    const MEMORY_512M: i64 = 512 * 1024 * 1024; // 512 MiB
    const MEMORY_1G: i64 = 1024 * 1024 * 1024; // 1 GiB
    const MEMORY_2G: i64 = 2 * 1024 * 1024 * 1024; // 2 GiB

    fn new_manager() -> FsManager {
        let mut m = FsManager::new(TEST_BASE).unwrap();
        m.destroy().unwrap();
